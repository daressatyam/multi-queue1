...
prior()
{
      ...
      int wait_time = 0, turnaround_time = 0, arrival_time[10], temp[10]; 
      float average_wait_time, average_turnaround_time;
      printf("priority scheduling :\t");
      printf("quantum time of 10 seconds:\t");
      int static time_quantum=10;
      limit=pp;
      for(i = 0; i < limit; i++)
      {
        temp[i]=pr[i].pb_t;
      ...
      { 
            ...
            { 
                  x--; 
                  printf("\nProcess[%d]\t\t%d\t\t %d\t\t\t %d", i + 1,pr[i].pb_t, total - pro[i].fa_t, total - pro[i].fa_t - pr[i].pb_t);
                  wait_time = wait_time + total - pro[i].fa_t - pr[i].pb_t; 
                  turnaround_time = turnaround_time + total - pro[i].fa_t; 
                  counter = 0; 
            } 
            if(i == limit - 1) 
            {
